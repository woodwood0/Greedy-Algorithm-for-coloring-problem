...
void sorting(list<int> adj_list[], int v, GraphData& data)               //找出最長的list
{
	data.START = clock();
	int iy, max = 0;
	for (iy = 1; iy < v; ++iy)
...
void processNode(list<int> adj_list[], int i, int v, int* result, bool* available)	//提取原先在 greedyColoring()中重複的程式碼
{
...
void greedyColoring(list<int> adj_list[], int v, int nodeiy, GraphData& data)         //用貪心演算法上色
{
	data.START = clock();
